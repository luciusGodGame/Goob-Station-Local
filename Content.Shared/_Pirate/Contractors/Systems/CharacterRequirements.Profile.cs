...
/// </summary>
[UsedImplicitly, Serializable, NetSerializable]
public sealed partial class CharacterLifepathRequirement : JobRequirement
{
    [DataField(required: true)]
        ...
        [NotNullWhen(false)] out FormattedMessage? reason)
    {
        ...
        var reasonString = Loc.GetString(
            ...
            ("lifepaths", $"[color={color}]{string.Join($"[/color], [color={color}]",
                Lifepaths.Select(s => Loc.GetString(protMan.Index(s).NameKey)))}[/color]"));

        reason = FormattedMessage.FromMarkup(reasonString);

        var isValid = profile.Lifepath != null && Lifepaths.Any(o => o == profile.Lifepath);
        return Inverted ? !isValid : isValid;
    }
}